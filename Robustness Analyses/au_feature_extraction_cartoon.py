...
def extract_au_features_for_task(emotion, output_dir="result/AU_Features_Cartoon", task_labels=None):
    """
    提取指定情绪下看动画片任务的AU特征
    ...
    for i, (_, row) in enumerate(demo_df.iterrows(), 1):
        ...
        try:
            ...
            # 获取任务帧和干扰帧位置
            task_frames, interference_frames = get_task_frames(subject, task_labels=task_labels)
            
            # 创建任务帧掩码（True表示是任务帧）
            task_mask = np.zeros(len(au_df), dtype=bool)
            if task_frames:
            ...
            # 有效掩码：任务帧 + 目标情绪 + 成功检测 + 非干扰帧
            valid_mask = (task_mask) & (emotion_mask) & (au_df['success'] == 1) & (interference_mask)
...
def extract_au_correlations_for_task(emotion, output_dir="result/AU_Features_Cartoon", task_labels=None):
    """
    提取指定情绪下看动画片任务的AU相关性特征
    ...
    print(f"\n开始提取{emotion}情绪下看动画片任务的AU相关性数据...")
...
def merge_au_intensities_cartoon(feature_dir="result/AU_Features_Cartoon",
                                 output_file="result/machine_learning/merged_au_intensities_cartoon.csv"):
    """
    合并看动画片任务的所有情绪的AU强度特征
...
def merge_au_correlations_cartoon(feature_dir="result/AU_Features_Cartoon",
                                  output_file="result/machine_learning/merged_au_correlations_cartoon.csv"):
    """
    合并看动画片任务的所有情绪的AU相关性特征
...
def main():
    ...
    # 定义输出目录
    output_dir = "result/AU_Features_Cartoon"
    ...
    print("=" * 60)
    for emotion in emotions:
        extract_au_features_for_task(emotion, output_dir=output_dir, task_labels=task_labels)
    ...
    print("=" * 60)
    for emotion in emotions:
        extract_au_correlations_for_task(emotion, output_dir=output_dir, task_labels=task_labels)
...
if __name__ == "__main__":
    ...
    # 合并所有特征用于模型分类
    print("=" * 80)
    print("开始合并AU特征用于模型分类（看动画片任务）")
    print("=" * 80)
    
    # 合并AU强度特征
    merged_intensities = merge_au_intensities_cartoon(
        feature_dir="result/AU_Features_Cartoon",
        output_file="result/machine_learning/merged_au_intensities_cartoon.csv"
    )
    
    # 合并AU相关性特征
    merged_correlations = merge_au_correlations_cartoon(
        feature_dir="result/AU_Features_Cartoon",
        output_file="result/machine_learning/merged_au_correlations_cartoon.csv"
    )
