"""
情绪特征提取 - 针对指定任务（看动画片）的特征提取
参考 emotion_analysis.ipynb 的特征提取方法
只提取特征，不进行统计分析和可视化
"""
...
def get_task_frames(subject, events_dir="result/Events", task_labels=None):
    """
    获取指定任务（看动画片）的帧位置，排除干扰帧
    ...
    events_dir : str
        事件文件目录
    task_labels : list
        要保留的任务标签列表，如['自选动画', '天鹅动画']
    
    Returns:
    --------
    tuple: (task_frames, interference_frames)
        task_frames: 任务帧位置列表 [[start_frame, end_frame], ...]
        interference_frames: 干扰帧位置列表 [[start_frame, end_frame], ...]
    """
    from lib.EventTracking import Events
    
    # 默认任务标签
    if task_labels is None:
        task_labels = ['自选动画', '天鹅动画']
    ...
    # 获取任务帧（看动画片的片段）
    task_events = events.filter_by_label(stay_labels=task_labels)
    task_frames = []
    if task_events.events:
        task_frames = task_events.get_frame_list(sr=50)  # 视频帧率为50fps
    
    # 获取干扰帧（口罩等）
    interference_events = events.filter_by_label(stay_labels=['口罩'])
    interference_frames = []
    if interference_events.events:
        interference_frames = interference_events.get_frame_list(sr=50)
    
    return task_frames, interference_frames
...
def extract_emotion_features_for_task(feature_type, emotion_dir="result/Emotions_smooth2", 
                                      output_dir="result/Emotion_Features_Cartoon", 
                                      task_labels=None, remove_outlier=True):
    """
    提取指定任务（看动画片）的情绪特征
    ...
    for i, file in enumerate(csv_files, 1):
        try:
            ...
            # 获取任务帧和干扰帧位置
            task_frames, interference_frames = get_task_frames(subject_name, task_labels=task_labels)
            
            # 创建任务帧掩码（True表示是任务帧）
            task_mask = np.zeros(len(df), dtype=bool)
            if task_frames:
                for start_frame, end_frame in task_frames:
                    # 确保索引在有效范围内
                    start_frame = max(0, min(start_frame, len(df) - 1))
                    end_frame = max(0, min(end_frame, len(df) - 1))
                    task_mask[start_frame:end_frame+1] = True
            
            # 创建干扰帧掩码（True表示非干扰帧）
            interference_mask = np.ones(len(df), dtype=bool)
            if interference_frames:
            ...
            # 筛选有效数据：任务帧 + 非干扰帧
            valid_mask = task_mask & interference_mask
    ...
    # 保存结果
    output_file = os.path.join(output_dir, f'{feature_type}_cartoon.csv')
...
def extract_all_emotion_features_for_task(emotion_dir="result/Emotions_smooth2",
                                          output_dir="result/Emotion_Features_Cartoon",
                                          task_labels=None,
                                          remove_outlier=True):
    """
    ...
    print("="*80)
    print("开始提取看动画片任务的所有情绪特征")
    print("="*80)
    
    all_results = {}
    
    for feature_type in feature_types:
        result_df = extract_emotion_features_for_task(
...
def merge_emotion_features_cartoon(feature_dir="result/Emotion_Features_Cartoon", 
                                   output_file="result/machine_learning/merged_emotion_features_cartoon.csv"):
    """
    合并看动画片任务的所有情绪特征，用于模型分类
    ...
    # 读取各种特征文件
    print("正在读取特征文件...")
    gev_df = pd.read_csv(os.path.join(feature_dir, 'GEV_cartoon.csv'), encoding='utf-8-sig')
    duration_df = pd.read_csv(os.path.join(feature_dir, 'Duration_cartoon.csv'), encoding='utf-8-sig')
    frequency_df = pd.read_csv(os.path.join(feature_dir, 'Frequency_cartoon.csv'), encoding='utf-8-sig')
    trans_df = pd.read_csv(os.path.join(feature_dir, 'TransWithOutSelf_cartoon.csv'), encoding='utf-8-sig')
    ...
    # Probability特征（可选，原文件中被注释掉了）
    # probability_file = os.path.join(feature_dir, 'Probability_cartoon.csv')
    # if os.path.exists(probability_file):
    #     print("合并Probability特征...")
    #     probability_df = pd.read_csv(probability_file, encoding='utf-8-sig')
    #     if 'Person' in probability_df.columns and '姓名' not in probability_df.columns:
    #         probability_df.rename(columns={'Person': '姓名'}, inplace=True)
    #     probability_feature_cols = [col for col in probability_df.columns if col not in available_base_cols]
    #     if '姓名' in probability_df.columns:
    #         result_df = result_df.merge(
    #             probability_df[['姓名'] + probability_feature_cols],
    #             on='姓名',
    #             validate='1:1'
    #         )
...
if __name__ == "__main__":
    # 如果只需要合并特征，可以直接运行合并函数
    # 如果需要提取特征，取消下面的注释
    
    # # 提取所有特征
    # all_results = extract_all_emotion_features_for_task(
    ...
    print("开始合并特征用于模型分类")
    print("="*80)
    merged_df = merge_emotion_features_cartoon(
        feature_dir="result/Emotion_Features_Cartoon",
        output_file="result/machine_learning/merged_emotion_features_cartoon.csv"
    )
